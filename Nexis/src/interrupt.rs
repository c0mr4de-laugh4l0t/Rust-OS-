...
// Phase-3 (cautious preemption): PIT init + deferred scheduling flag checked at safe points.
...
mod pit; // PIT driver (cautious mode)
...
fn kernel_main(_boot_info: &'static BootInfo) -> ! {
    // init IDT + PIC (load IDT entries)
    interrupts::init_idt();
    interrupts::remap_pic();

    // Initialize PIT in cautious mode (fires IRQ0, handler sets NEED_RESCHED flag)
    pit::init(50); // 50 Hz tick
    crate::vga::vprintln!("PIT initialized (50 Hz, cautious mode)");

    // enable interrupts globally AFTER PIT is configured
    ...
    // init VGA & clear
    {
        ...
        vw.clear_screen();
        vw.write_str("\n=== IronVeil / Nexis (VGA IRQ keyboard + PMM + Scheduler + PIT) ===\n");
    ...
    // demo_task: quick ticking test that now checks the NEED_RESCHED flag periodically
    extern "C" fn demo_task() {
        ...
        loop {
            ...
            // do some work (busy loop) but periodically check for reschedule requests
            for _ in 0..200_000 {
                core::hint::spin_loop();
                // check the PIT reschedule flag at a safe point
                crate::scheduler::check_and_schedule();
            }

            // cooperative yield as fallback
...
fn shell_loop() -> ! {
    ...
    loop {
        // Check the PIT reschedule flag at a safe point before printing prompt
        crate::scheduler::check_and_schedule();
        ...
        // Check reschedule again right after input is received (safe point)
        crate::scheduler::check_and_schedule();
